...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
      ...
      if (currentUser) {
        profileUnsubscribe = onSnapshot(doc(db, 'users', currentUser.uid), (docSnap) => {
          if (docSnap.exists()) {
            ...
            if (currentUser.email === 'cerezvincent1@gmail.com' && currentUser.emailVerified) {
              setRealUserProfile({
                uid: currentUser.uid,
  ...
  const realAdminStatus = realUserProfile?.role === 'admin' || (realUser?.email === 'cerezvincent1@gmail.com' && realUser?.emailVerified);
  ...
  const isAdmin = effectiveUserProfile?.role === 'admin' || (effectiveUser?.email === 'cerezvincent1@gmail.com' && effectiveUser?.emailVerified) || false;
